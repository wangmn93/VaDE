...
classifier = partial(models.cnn_classifier,out_dim=10)
# classifier = partial(models.encoder, z_dim = z_dim)
...
r_mean = classifier(real, reuse=False, keep_prob=.5)
# f_mean = classifier(fake)

r_p = tf.nn.softmax(r_mean)
# f_p = tf.nn.softmax(f_mean)

#discriminator loss
d_loss = -1 * (mar_entropy(r_p) - cond_entropy(r_p))   # Equation (7) upper
...
#predict
embed_mean = classifier(real, keep_prob=1.)
